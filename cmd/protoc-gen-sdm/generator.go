...
func generateModels(gen *protogen.Plugin, file *protogen.File) {
	...
	g.P("// Code generated by protoc-gen-sdm. DO NOT EDIT.")
	g.P()
	...
	for _, msg := range file.Messages {
		...
		for _, field := range msg.Fields {
			...
			if opts.PrimaryKey || opts.Pii || opts.QueryIndex {
				...
				// Not in PII table, must be in Chain table (or just missing? Requirement says 'seller: acc_id' which isn't PII nor Hashed explicitly in proto, but let's assume standard fields are in PII or just unsupported for now?
				// Wait, the requirements Example Invoice object:
				// seller: "acc_id" (no annotations).
				// chain object has it.
				// pii object DOES NOT have it.
				// So non-annotated fields go to Chain?
				// "Pii data never goes on the chain. It stays only in the database."
				// "chain_invoices table - a projection of invoices data on chain"
				// "invoice object on chain ... hashed_seller_gst ... seller ..."

				// Assumption: Non-PII fields are stored in the Chain table too?
				// Actually, the example shows 'chain_invoices' is a KV store for "hashed_seller_gst".
				// BUT the 'invoice object on chain' implies the smart contract has these fields.
				// The *SQL View* is constructing data from the local DB tables.
				// The local DB 'chain_invoices' table seems to store projections of what's on chain.
				// For the View, we need to reconstruct the full object.
				// If a field is NOT in PII table, where is it?
				// If it's on chain, maybe we store it in `chain_invoices` as well?
				// The example `chain_invoices` table structure only shows `hashed_seller_gst`.
				// But implementation details for 'seller' (plain string) are missing in the 'chain_invoices' local table example.
				// However, `invoices` view shows `seller`.
				// I'll assume for this task: Non-PII fields are ALSO stored in `pii_invoices` for simplicity of reconstruction,
				// OR they are stored as separate keys in `chain_invoices`.
				// Let's assume NON-PII fields are stored in PII table (but not marked as PII) OR we replicate them.
				// Re-reading: "Pii data never goes on the chain".
				// "Invoice object ... 2 tables would be created on db ... pii_invoices ... chain_invoices".
				// `pii_invoices` example excludes `seller` and `amount`.
				// `chain_invoices` example only shows `hashed_seller_gst`.
				// But `invoices` view includes `seller` and `amount`.
				// This implies `seller` and `amount` must be fetchable from somewhere.
				// Maybe `chain_invoices` stores them too?
				// "chain_invoices table - a projection of invoices data on chain"
				// So `chain_invoices` should store `seller` -> `acc_id_0123`.

				// I will treat ANY field that is NOT PII as "Chain Data" which is stored in `chain_invoices`.
				// AND `pii` fields are stored in `pii_invoices`.
				// `hashed` fields are stored in `chain_invoices` as `hashed_<name>`.

				// Wait, `pii_invoices` example shows `invoice_number` (PII).
				// `invoice_number` is PII.

				// Okay, strategy:
				// 1. PII table gets: PK, and (PII=true).
				// 2. Chain table gets: (PII=false) fields, and Hashed versions of (Hashed=true) fields.
				//    Chain table is EAV (Entity Attribute Value).

				// View construction:
				// Base: PII table (p).
				// Joins: Left Join Chain table for each relevant field.

				// But non-annotated fields like `seller`?
				// "seller_gst: "gst012", //pii //hashed"
				// "seller: "acc_id" (no annotation)"
				// I will implement:
				// - If (PII): Go to PII table.
				// - If (!PII): Go to Chain table (as Key=ID, FieldName=Name, Value=Value).
				// - If (Hashed): Go to Chain table (as Key=ID, FieldName=hashed_Name, Value=Hash).

				// So, back to View construction.
				if opts.Pii {
					selects = append(selects, fmt.Sprintf("p.%s", colName))
				} else {
					// It's a chain field
